 ...
 * Copyright(c) 2015 Eugene Bolshakov <pub@relvarsoft.com>
 *
...
struct cy8c9540a {
	...
	/* last PWM parameters per PWM for speed up */
	int pwm_last_period_ns[CY8C9540A_NPWM];
	int pwm_last_period_min[CY8C9540A_NPWM];
	int pwm_last_period[CY8C9540A_NPWM];
	int pwm_last_clk_src[CY8C9540A_NPWM];
	int pwm_last_clk_div[CY8C9540A_NPWM];
	ktime_t pwm_last_time[CY8C9540A_NPWM];
};


/* clock sources for PWM */
#define CLK_SOURCE_32_kHz		0
#define CLK_SOURCE_24_MHz		1
#define CLK_SOURCE_1p5_MHz		2
#define CLK_SOURCE_93p75_kHz	3
#define CLK_SOURCE_367p6_Hz		4
#define CLK_SOURCE_previous		5

struct cy8c9540a_clock_rate {
	u32 freq_rate_min;
	u32 freq_rate_max;
	u32 period_rate_max;
	u32 period_rate_min;
...
/* Clock source selection for PWM */
static const struct cy8c9540a_clock_rate cy8c9540a_clock_source[] = {
	{94117,    24000000, 10625,     41,    CLK_SOURCE_24_MHz,    0 },
	{5882,     1500000,  170010,    666,   CLK_SOURCE_1p5_MHz,   0 },
	{367,      93750,    2724795,   10667, CLK_SOURCE_93p75_kHz, 0 },
	{125,      32000,    8000000,   31250, CLK_SOURCE_32_kHz,    0 },
	{1,        93750,    690880000, 10667, CLK_SOURCE_367p6_Hz,  1 },
};
				   ...
				   unsigned gpio, int val)
{
	...
	if(tmpval != dev->outreg_cache[port]){
					...
					unsigned mode)
{
	...
	if (ret < 0) {
		dev_err(&client->dev, "can't read pin drive\n");
		goto end;
	}
...
static void cy8c9540a_pwm_invalidate_cache(struct pwm_chip *chip, struct pwm_device *pwm)
{
	struct cy8c9540a *dev = container_of(chip, struct cy8c9540a, pwm_chip);

	dev->pwm_last_clk_src[pwm->pwm]    = -1;
	dev->pwm_last_clk_div[pwm->pwm]    = 0;
	dev->pwm_last_period[pwm->pwm]     = 0;
	dev->pwm_last_period_ns[pwm->pwm]  = 0;
	dev->pwm_last_period_min[pwm->pwm] = 0;
}
...
{
	int ret = 0, i;
	int period = 0, period_min = 0, duty = 0, clk_src = -1, period_err = 1000000000, clk_div = 0;
	int tmp_duty, tmp_period, tmp_period_min, tmp_period_err, tmp_clk_div;
	struct cy8c9540a *dev =
	 ...
	 */
	/* if more 24 MHz or negative */
	if (period_ns < 41) {
		dev_err(&client->dev, "PWM: period=%d must be positive and above 40 ns \n",period_ns);
		return -EINVAL;
	}
	/* if less 1,4 Hz */
	if (period_ns > 690880001) {
		dev_err(&client->dev, "PWM: period=%d must be less 690880001 ns \n",period_ns);
		return -EINVAL;
	}

	/* select a clock source with minimum period error */
	if(dev->pwm_last_period_ns[pwm->pwm] == period_ns){
		/* take from cache */
		clk_src = dev->pwm_last_clk_src[pwm->pwm];
		clk_div = dev->pwm_last_clk_div[pwm->pwm];
		period = dev->pwm_last_period[pwm->pwm];
		period_min = dev->pwm_last_period_min[pwm->pwm];
		duty = duty_ns / period_min;
		if(clk_div) { duty = duty / clk_div; }
	}else{
		for(i=0; i<ARRAY_SIZE(cy8c9540a_clock_source); i++){
			if(period_ns >= cy8c9540a_clock_source[i].period_rate_min && period_ns <= cy8c9540a_clock_source[i].period_rate_max){
				tmp_period_min = cy8c9540a_clock_source[i].period_rate_min;
				tmp_period = period_ns / tmp_period_min;
				tmp_duty = duty_ns / tmp_period_min;
				tmp_clk_div = cy8c9540a_clock_source[i].clk_flags;
				if(tmp_clk_div){
					if(tmp_period <= PWM_MAX_PERIOD){
						tmp_clk_div = 1;
					}else{
						int j, tmpp, tmpe, p=PWM_MAX_PERIOD, err=1000000000;
						for(j=1; j < 128; j++){
							tmpp = tmp_period / j;
							tmpe = period_ns - tmp_period_min * tmpp * j;
							if(err > tmpe && tmpp <= PWM_MAX_PERIOD) {
								err = tmpe;
								p = tmpp;
							}
						}
						tmp_clk_div = p;
						tmp_period = tmp_period / tmp_clk_div;
						tmp_duty = tmp_duty / tmp_clk_div;
					}
					tmp_period_err = period_ns - tmp_period * tmp_clk_div * cy8c9540a_clock_source[i].period_rate_min;
				}else{
					tmp_period_err = period_ns - tmp_period * cy8c9540a_clock_source[i].period_rate_min;
				}
				if(tmp_period_err < 0) {tmp_period_err = -tmp_period_err;}
				if(period_err > tmp_period_err){
					period = tmp_period;
					duty = tmp_duty;
					period_err = tmp_period_err;
					clk_src = cy8c9540a_clock_source[i].clk_source;
					clk_div = tmp_clk_div;
					period_min = tmp_period_min;
				}
			}
		}
	}
	...
	if(clk_src < 0){
		dev_err(&client->dev, "PWM: cannot select clock source for period=%d \n",period_ns);
		dev->pwm_last_clk_src[pwm->pwm]    = -1;
		dev->pwm_last_clk_div[pwm->pwm]    = 0;
		dev->pwm_last_period[pwm->pwm]     = 0;
		dev->pwm_last_period_ns[pwm->pwm]  = 0;
	...
	if(dev->pwm_last_period_ns[pwm->pwm] != period_ns){
		dev->pwm_last_clk_src[pwm->pwm]    = clk_src;
		dev->pwm_last_clk_div[pwm->pwm]    = clk_div;
		dev->pwm_last_period_ns[pwm->pwm]  = period_ns;
		dev->pwm_last_period[pwm->pwm]     = period;
		dev->pwm_last_period_min[pwm->pwm] = period_min;

		ret = i2c_smbus_write_byte_data(client, REG_PWM_CLK, (u8)clk_src);
		if (ret < 0) {
			dev_err(&client->dev, "can't write to REG_PWM_CLK\n");
			goto end;
		}

		if(clk_src == CLK_SOURCE_367p6_Hz){
			ret = i2c_smbus_write_byte_data(client, REG_PWM_DIV, (u8)clk_div);
			if (ret < 0) {
				dev_err(&client->dev, "can't write to REG_PWM_DIV\n");
				goto end;
			}
		}
...
static int cy8c9540a_pwm_enable(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	cy8c9540a_pwm_invalidate_cache(chip,pwm);
